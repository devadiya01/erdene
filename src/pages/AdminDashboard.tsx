...
import { BarChart2, Users, DollarSign, Loader, Search, Filter, Calendar } from 'lucide-react';
  ...
  async function fetchData() {
    try {
      const [productsData, requestsData, matchesData] = await Promise.all([
        supabase
          .from('products')
          .select('id, title, price, quantity, unit, category, seller:users(email)')
          .eq('status', 'active'),
        supabase
          .from('requests')
          .select('id, title, budget, quantity, unit, category, delivery_date, buyer:users(email)')
          .eq('status', 'active'),
        supabase
          .from('matches')
          .select(`
      ...
      const totalRevenue = matchesData.data?.reduce((sum, match) => sum + (match.fee || 0), 0) || 0;
      ...
      const matchRate = activeProducts > 0 ? (totalMatches / activeProducts) * 100 : 0;

      // Calculate category statistics
      const categoryStats = productsData.data?.reduce((acc: { [key: string]: number }, product) => {
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold">${stats.averageFee.toFixed(2)}</h3>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold">{stats.matchRate.toFixed(1)}%</h3>
            </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form onSubmit={handleCreateMatch} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Бүтээгдэхүүн сонгох</label>
              <select
                required
              ...
              >
                ...
                {products.map((product) => (
                  <option key={product.id} value={product.id}>
                    {product.title} - ${product.price} ({product.quantity} {product.unit}) - {product.seller.email}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Хүсэлт сонгох</label>
              <select
                required
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
              ...
              >
                ...
                {requests.map((request) => (
                  <option key={request.id} value={request.id}>
                    {request.title} - ${request.budget} ({request.quantity} {request.unit}) - {request.buyer.email}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Шимтгэлийн хэмжээ</label>
              <input
                type="number"
                required
                min="0"
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Үнэ: ${match.product.price}</p>
                  <p className="text-sm text-gray-500">
                    Тоо хэмжээ: {match.product.quantity} {match.product.unit}
                  </p>
                  <p className="text-sm text-gray-500">Борлуулагч: {match.product.seller.email}</p>
                </div>
                <div>
                  <h3 className="font-semibold">Хүсэлт</h3>
                  <p>{match.request.title}</p>
                  <p className="text-sm text-gray-500">Төсөв: ${match.request.budget}</p>
                  <p className="text-sm text-gray-500">
                    Тоо хэмжээ: {match.request.quantity} {match.request.unit}
                  </p>
                  <p className="text-sm text-gray-500">Худалдан авагч: {match.request.buyer.email}</p>
                </div>
                <div>
                  ...
                  <p className="font-bold text-blue-600 text-lg mt-2">Шимтгэл: ${match.fee}</p>
                  <div className="mt-4">
                    <select
        ...
        {filteredMatches.length === 0 && (
          <p className="text-gray-500 text-center py-8">Одоогоор тохиргоо байхгүй байна!</p>
        )}
      </div>
    </div>
  );
